<div class="card-body">
    ...
    <div class="card-header with-border pl-0 pb-1">
        <span class="col-form-label text-bold">Kinerja Karyawan</span>
    </div>
    <br>
            ...
            <select name="staff_id" class="form-control select2 @error('staff_id') is-invalid @enderror">
                ...
                @foreach ($staff as $item)
                    <option value="{{ $item->id }}"
                        {{ $item->id == old('staff_id', $kinerja->staff_id ?? '') ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('staff_id')
    ...
    <div class="form-group row">
        ...
        <div class="col-12 col-md-5 col-lg-5">
            <select name="status" class="form-control select2 @error('status') is-invalid @enderror">
                <option value=""></option>
                 @foreach (['kurang', 'cukup', 'baik', 'sangatbaik'] as $value)
                    <option value="{{ $value }}" {{ old('status', $kinerja->status ?? '') == $value ? 'selected' : '' }}>
                        {{ $value }}
                    </option>
                @endforeach
            </select>
    ...
    <div class="form-group row">
        <label class="col-md-4 col-xs-4 col-form-label justify-flex-end">Bulan <span class="text-danger">*</span></label>
        <div class="col-12 col-md-5 col-lg-5">
            <input type="month" name="month" class="form-control @error('month') is-invalid @enderror"
                value="{{ old('month', $kinerja->month ?? '') }}" autocomplete="off">
            @error('month')
                <span class="invalid-feedback" role="alert">
    ...
    <div class="form-group row">
        ...
        <div class="col-12 col-md-5 col-lg-5">
            <textarea name="keterangan" class="form-control @error('keterangan') is-invalid @enderror"
                placeholder="Masukan keterangan..">{{ old('keterangan', $kinerja->keterangan ?? '') }}</textarea>
            @error('keterangan')
                <span class="invalid-feedback" role="alert">

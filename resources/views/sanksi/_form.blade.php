<div class="card-body">
            ...
            <select name="staff_id" class="form-control select2 @error('staff_id') is-invalid @enderror">
                ...
                @foreach ($staff as $item)
                    <option value="{{ $item->id }}"
                        {{ $item->id == old('staff_id', $sanksi->staff_id ?? '') ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('staff_id')
        ...
        <div class="col-12 col-md-5 col-lg-5">
            <select name="position_id" class="form-control select2 @error('position_id') is-invalid @enderror">
                ...
                @foreach ($posisi as $item)
                    <option value="{{ $item->id }}"
                        {{ $item->id == old('position_id', $sanksi->position_id ?? '') ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('position_id')
    ...
    <div class="form-group row">
        ...
        <div class="col-12 col-md-5 col-lg-5">
            <textarea name="keterangan" class="form-control @error('keterangan') is-invalid @enderror"
                placeholder="Masukan keterangan..">{{ old('keterangan', $sanksi->keterangan ?? '') }}</textarea>
            @error('keterangan')
                <span class="invalid-feedback" role="alert">
    ...
    <div class="form-group row">
        <label class="col-md-4 col-xs-4 col-form-label justify-flex-end">Status <span
                class="text-danger">*</span></label>
        <div class="col-12 col-md-5 col-lg-5">
            <select name="peringatan_id" class="form-control select2 @error('peringatan_id') is-invalid @enderror">
                <option value=""></option>
                @foreach ($peringatan as $item)
                    <option value="{{ $item->id }}"
                        {{ $item->id == old('peringatan_id', $sanksi->peringatan_id ?? '') ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('peringatan_id')
                <span class="invalid-feedback" role="alert">
                    <strong>{{ $errors->first('peringatan_id') }}</strong>
                </span>
            @enderror
        </div>
    </div>

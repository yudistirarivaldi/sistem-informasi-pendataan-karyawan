                        ...
                        <div class="card">
                            <div class="card-header bg-light">
                                <h3 class="card-title back-top" style="margin-top: 5px;">
                                    <a href="{{ route('salary.index') }}" title="Kembali" data-toggle="tooltip"
                                            ...
                                            <select name="filter" class="form-control input-sm select2">
                                                ...
                                                @foreach ($periode as $item)
                                                    <option value="{{ $item->periode }}"
                                                        ...
                                                        {{ strtoupper($item->periode) }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <tbody>
                                            @forelse ($salary as $item)
                                                <tr style="line-height: 1;">
                                                    ...
                                                    <td>
                                                        <span
                                                            class="badge {{ $item->status_gaji == 'Lunas' ? 'badge-success' : 'badge-danger' }}">{{ $item->status_gaji ?? 'Belum Lunas' }}</span>
                                                    </td>
                                                    <td>{{ $item->jumlah_overtime }}</td>
                                                    <td>Rp. {{ number_format($item->uang_overtime, 0, ',', '.') }} / Jam
                                                    </td>
                                                    @if ($staff->position->status == 'Staff')

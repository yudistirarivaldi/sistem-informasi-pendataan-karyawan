<div class="card-body">
                ...
                <select name="staff_id" class="form-control select2 @error('staff_id') is-invalid @enderror">
                    ...
                    @foreach ($staff as $item)
                        <option value="{{ $item->id }}"
                            {{ $item->id == old('staff_id', $cuti->staff_id ?? '') ? 'selected' : '' }}>
                            {{ $item->name }}</option>
                    @endforeach
                </select>
            @else
    ...
    <div class="form-group row">
        <label class="col-md-4 col-xs-4 col-form-label justify-flex-end">Tgl. Cuti <span
                class="text-danger">*</span></label>
        <div class="col-12 col-md-5 col-lg-5">
            @if (isset($cuti))
                <input type="text" class="form-control pull-right datepicker" name="estimasi"
                    value="{{ date('m/d/Y', strtotime($cuti->tgl_mulai)) . ' - ' . date('m/d/Y', strtotime($cuti->tgl_selesai)) }}"
                    required autocomplete="off" onkeypress="return false">
            @else
                <input type="text" class="form-control pull-right datepicker" name="estimasi"
                    value="{{ old('estimasi') }}" required autocomplete="off" onkeypress="return false">
            @endif
            @error('estimasi')

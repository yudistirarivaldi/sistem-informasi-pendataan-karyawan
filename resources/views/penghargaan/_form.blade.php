<div class="card-body">
    ...
    <div class="card-header with-border pl-0 pb-1">
        <span class="col-form-label text-bold">Penghargaan Karyawan</span>
    </div>
    <br>
            ...
            <select name="staff_id" class="form-control select2 @error('staff_id') is-invalid @enderror">
                ...
                @foreach ($staff as $item)
                    <option value="{{ $item->id }}"
                        {{ $item->id == old('staff_id', $penghargaan->staff_id ?? '') ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('staff_id')
        ...
        <div class="col-12 col-md-5 col-lg-5">
            <select name="position_id" class="form-control select2 @error('position_id') is-invalid @enderror">
                ...
                @foreach ($posisi as $item)
                    <option value="{{ $item->id }}"
                        {{ $item->id == old('position_id', $penghargaan->position_id ?? '') ? 'selected' : '' }}>
                        {{ $item->name }}</option>
                @endforeach
            </select>
            @error('position_id')
    ...
    <div class="form-group row">
        ...
        <div class="col-12 col-md-5 col-lg-5">
            <textarea name="keterangan" class="form-control @error('keterangan') is-invalid @enderror"
                placeholder="Masukan keterangan..">{{ old('keterangan', $penghargaan->keterangan ?? '') }}</textarea>
            @error('keterangan')
                <span class="invalid-feedback" role="alert">

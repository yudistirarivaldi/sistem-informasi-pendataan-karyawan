<div class="card-body">
    ...
    <div class="form-group row">
        <label hidden class="col-md-4 col-xs-4 col-form-label justify-flex-end">KODE ABSENSI <span
                class="text-danger">*</span></label>
        <div class="col-12 col-md-5 col-lg-5">
            <input hidden type="text" class="form-control {{ $errors->has('code') ? ' is-invalid' : '' }}"
                name="code" value="{{ $code ?? '' }}" readonly required />
            @if ($errors->has('code'))
    ...
    <div class="form-group row">
        <label class="col-md-4 col-xs-4 col-form-label justify-flex-end">Periode <span
                class="text-danger">*</span></label>
        <div class="col-12 col-md-5 col-lg-5">
            <select name="periode" class="form-control select2 @error('periode') is-invalid @enderror" id=""
                required>
                <option value=""></option>
                @for ($index = 0; $index <= 12; $index++)
                    <option value="{{ $month[$index] }}"
                        {{ $month[$index] == old('periode', $month[date('n')] ?? '') ? 'selected' : '' }}>
                        {{ $month[$index] . ', ' . date('Y') }}</option>
                @endfor
            </select>
            @error('periode')
                <span class="invalid-feedback" role="alert">
    ...
    <div class="form-group row">
        ...
        <div class="col-12 col-md-5 col-lg-5">
            <div class="input-group">
                ...
                <input type="text"
                    class="form-control pull-right datepicker{{ $errors->has('tanggal') ? ' is-invalid' : '' }}"
                    name="tanggal" placeholder="31/04/2019" autocomplete="off" required onkeypress="return false" />
            </div>
...
<div class="card-footer">
    <div class="float-right">
        <div class="form-group mb-0">
            ...
            <button type="submit" class="btn btn-primary" name="btnIn"><i class="fas fa-arrow-right mr-1"></i>
                Next</button>
        </div>
    </div>
</div>
